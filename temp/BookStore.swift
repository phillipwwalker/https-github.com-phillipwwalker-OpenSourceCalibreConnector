...
func sortAuthorByCount(_ author: [Author]) -> [Author] {
    ...
    func loadBookStore(){
        ...
        persistanceHandler.load_books(book_store: self)
        persistanceHandler.loadReadingList(book_store: self)
        
        
        sortedBooks = reverseSortBooksByDate(allBooks)
        print("sorted title[1] =",sortedBooks[1].title)
        print("title[1] =",allBooks[1].title)
        allBooks = sortedBooks
        //let loadCalibre = LoadCalibre()
        //loadCalibre.readCalibre()
    }
    */
